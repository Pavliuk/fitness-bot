...
1. Отримайте `API_ID` і `API_HASH` на https://my.telegram.org → *API
   development tools*.
2. Дізнайтесь свій Telegram user id (напр. через [@userinfobot](https://t.me/userinfobot))
   і впишіть його в `.env` як `ADMIN_IDS` (через кому, якщо їх декілька).
3. Заповніть у `.env`: `TG_API_ID`, `TG_API_HASH`.
4. Виконайте одноразовий вхід (запитає номер телефону й код із Telegram):
   ```bash
...
5. Запустіть бота і в чаті з ним (як адмін з `ADMIN_IDS`) виконайте:
   ```
